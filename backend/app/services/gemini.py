...
def build_extraction_prompt(columns: List[ColumnConfig], categories: List[CategoryConfig] = None, corrections: list = None, cost_categories: list = None) -> str:
    ...
    if cost_categories:
        ...
        fields_desc["cost_category"] = (
            ...
            "IMPORTANT: Classify the invoice as a WHOLE using ALL line items — one category per invoice. "
            "DEFINITIVE vendor rules for this FTTH project (use vendor name as primary signal): "
            "- Westower Communications → always Fiber Build > Aerial Installation. "
            "- Bell Canada → always Make Ready (permits/engineering & admin). "
            "- Nokia Canada → always Electronics (OLT/chassis/optics/ONTs/software). "
            "- Algoma Power / Espanola Hydro / Hydro One → always Fiber Build > Construction Support. "
            "- Highway Corridor Management / MTO → always Make Ready (permit fees). "
            "- Huron Central Railway (HCRY) → always Fiber Build > Drawings/Design and Engineering. "
            "- Beameo Services / Cyient Canada / Phasor Engineering → always Fiber Build > Drawings/Design and Engineering. "
            "- MACC4D / Eion → always Fiber Build > Underground Installation. "
            "- SC360 Inc. → always Fiber Build > Construction Support. "
            "- Valard-Hydro → always Make Ready. "
            "- Valard-Telco: Make Ready for (setup/travel for make-ready, guard cable, anchor hardware, conduit supply, "
            "  pole planting, rock breaker, traffic control, civil works under utility/ISED permits); "
            "  Fiber Build > Aerial Installation for (new strand installation, overlash, PM/scheduler hours); "
            "  Fiber Build > Underground Installation for (explicit underground fiber construction — bore/plow for fiber). "
            "- CBSA → Material > Misc. "
            "(string, required)"
        )
