...
def build_extraction_prompt(columns: List[ColumnConfig], categories: List[CategoryConfig] = None, corrections: list = None, cost_categories: list = None) -> str:
    ...
    if cost_categories:
        ...
        fields_desc["cost_category"] = (
            ...
            "1. If majority of lines are hours/perdiem/travel/wages with no site work → Payroll. "
            "2. If invoice is for hydro/electricity/utility/power/kWh/Hydro One → Fiber Build (site utility). "
            "3. If invoice is for optical/network EQUIPMENT (OLT, ONT, chassis, optical modules, transceivers) → Electronics. "
            "4. If invoice is for preliminary/prep work ONLY (permits, engineering studies, clearing, pole transfers, make-ready prep) → Make Ready. "
            "5. Everything else → Fiber Build. This includes: "
            "   supply of construction materials (conduit, guard cable, anchor hardware, vaults, fibre); "
            "   supply-and-install work; civil vault installation; underground boring; aerial work; "
            "   pole planting; drops; any construction activity including material-only invoices for the build. "
            "NOTE: The 'Material' category should almost never be used for this FTTH project — if in doubt use Fiber Build. "
            "(string, required)"
        )
